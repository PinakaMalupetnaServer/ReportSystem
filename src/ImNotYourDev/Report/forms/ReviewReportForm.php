...
class ReviewReportForm extends MenuForm
{
    ...
    public function __construct(array $report)
    {
        ...
        $options = [
            ...
            new MenuOption("§cBan player (NOTICE)"),
        ...
        parent::__construct($title, $text, $options, function (Player $player, $data)  : void {
            $target = Report::getInstance()->getServer()->getPlayerExact($this->report["player"]);
            if($data == 0){
                ...
                if($target == true){
                    ...
                    $player->sendForm($this); // idk if forms is implemented.
                }
            }elseif($data == 1){
                ...
                $player->sendForm($this); */ // Not Compatible to API4 as of now
                $player->sendMessage(Report::getInstance()->prefix . "§eNot Compatible yet to API4");
